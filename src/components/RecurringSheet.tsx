...
export const RecurringSheet: React.FC<RecurringSheetProps> = ({ initial, onClose }) => {
  ...
  const [daysOfMonth, setDaysOfMonth] = useState<number[]>(
    Array.isArray(initial?.dayOfMonth) 
      ? initial!.dayOfMonth 
      : [initial?.dayOfMonth ?? new Date().getDate()]
  );
  ...
  const isValid = name.trim().length > 0 && Number(amount) > 0 && (frequency !== 'weekly' || daysOfWeek.length > 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await upsertRecurring({
        ...
        type,
        frequency,
        dayOfMonth: frequency === 'monthly' ? daysOfMonth : undefined,
      ...
      console.error('[MONA] Error saving recurring expense:', error);
    } finally {
      setSaving(false);
    }
  };
      ...
      >
        <div className="sticky top-0 bg-white/90 backdrop-blur-xl border-b border-zinc-100 px-5 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500">Recordatorio automático.</p>
          </div>
          <button
            onClick={onClose}
            aria-label="Cerrar"
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            <label htmlFor="rec-name" className="text-xs font-bold uppercase tracking-widest text-zinc-400 ml-1">
              Nombre
            </label>
            <input
              id="rec-name"
          ...
          <div>
            <label htmlFor="rec-amount" className="text-xs font-bold uppercase tracking-widest text-zinc-400 ml-1">
              Monto
            </label>
          ...
          <div>
            <label className="text-xs font-bold uppercase tracking-widest text-zinc-400 ml-1 block mb-2">
              Categoría
            </label>
          ...
          {frequency === 'monthly' && (
            <div>
                     ...
                     <button
                      ...
                      onClick={() => {
                        setDaysOfMonth(prev => {
                          ...
                          if (prev.length >= 2) {
                            return [prev[1], day].sort((a, b) => a - b);
                          }
              ...
              <p className="text-sm text-zinc-500 mt-1.5 ml-1">
          ...
          {frequency === 'weekly' && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-bold uppercase tracking-widest text-zinc-400 ml-1 block">
                  Días de la semana
                </label>
                ...
                {WEEKDAY_PICKER_ORDER.map((d) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'py-2.5 sm:py-2.5 px-0.5 rounded-lg text-sm sm:text-xs font-semibold tracking-tight transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900/30 truncate flex items-center justify-center',
                        active
                          ? 'bg-zinc-900 text-white shadow-sm'
              ...
              {daysOfWeek.length === 0 && (
                <p className="text-sm text-red-500 mt-1.5 ml-1">Selecciona al menos un día.</p>
              )}
            </div>
          )}

          <div>
            <label htmlFor="rec-time" className="text-xs font-bold uppercase tracking-widest text-zinc-400 ml-1">
          ...
          <button
            ...
            className="w-full py-3.5 rounded-2xl bg-zinc-900 text-white font-semibold shadow-lg shadow-black/10 hover:bg-zinc-800 active:bg-zinc-700 disabled:opacity-50 disabled:bg-zinc-300 disabled:text-zinc-500 transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
            <Check size={16} />
            {saving ? 'Guardando…' : initial ? 'Guardar cambios' : type === 'income' ? 'Crear ingreso fijo' : 'Crear gasto fijo'}
          </button>
        </form>
      </motion.div>
    </motion.div>
